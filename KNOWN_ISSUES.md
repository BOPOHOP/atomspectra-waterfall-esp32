...
на плате (`current.bin`) начинается заново. На стенде 14.09 так пропали 94 ч набора; уцелел
последний резервный снимок (issue #52) — потеряно время от него до перезагрузки. Кнопка об этом
не предупреждает и снимок перед командой не делает. **Перед перезагрузкой прибора сохраните
спектр вручную** («Сохранить» на странице «Спектр»).

### Сторож набора не действует, когда прибором управляет TCP-клиент
...
RX-сторож видел непрерывный 2-байтовый статус FTDI и считал связь живой. Если же команды шлёт
приложение AtomSpectra через TCP-мост, шлюз не знает, остановлен ли набор намеренно, и сторож
молчит — иначе он перебивал бы «Стоп» пользователя. Набор, запущенный не шлюзом, сторожем тоже
не охраняется.
