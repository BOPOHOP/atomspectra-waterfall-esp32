...
профили pile-up и термокомпенсации. Эти значения видны в ответе прибора на команду `-inf`.
...
**Обходной путь / восстановление:** подключить прибор к **штатному приложению AtomSpectra
от производителя** (KB Radar) и выполнить штатную перенастройку/восстановление профиля
детектора. Для этого ESP32-шлюз не нужен — прибор можно подключить к ПК напрямую (FTDI,
600000 бод).

**Рекомендация на будущее:** шлюз умеет **читать** `-inf` (показывает поля в Web UI), но
не делает их резервную копию. Имеет смысл при настроенном приборе один раз сохранить полный
рабочий ответ `-inf` (эталонный «слепок» DSP-конфигурации) — тогда повторный сброс можно
будет сверить и корректно восстановить.
...
**Контекст:** ветка `rec-11-autonomous-recording`.
...
непротестированной; поэтому фича пока живёт в ветке, а не в `main`.
